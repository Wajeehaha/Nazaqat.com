...
const Order = require('./models/Order'); // Import the Order model (fixed case sensitivity)
...
// Set up multer for file uploads - Using memory storage for Vercel
// Note: For production, files should be uploaded to cloud storage (Cloudinary/AWS S3)
...
app.post('/api/nails', upload.single('image'), async (req, res) => {
    try {
        ...
        // Note: In serverless environment, files are stored in memory
        // For production, implement cloud storage (Cloudinary/AWS S3)
...
app.post('/api/logo', upload.single('image'), async (req, res) => {
    try {
        // Note: In serverless environment, implement cloud storage for production
